...
export class ConversationService {
    ...
    static userUnblockConversation = async (req: Request, res: Response) => {
        try {
            ...
            if(conversationId == null || creatorId == null)
            {
                throw new BadRequestError(`conversationId and creatorId are required`);
            }            
            // check if the user is creator of the conversation
            const participant = await ParticipantRepo.getParticipantByConversation(creatorId, conversationId);
            if(!participant){
                throw new NoEntryError(`Participant not found`);
            }
            if(!participant.isCreator)
            {
                throw new ForbiddenError(`You are not creator of the conversation`);
            }
            // check if the conversation is exist
            const existingConversation = await ConversationRepo.getConversation(conversationId);
            if (!existingConversation) {
